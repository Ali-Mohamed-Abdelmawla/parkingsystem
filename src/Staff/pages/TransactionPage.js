...
function TransactionPage({ darkMode }) {
  ...
  useEffect(() => {
    if (accessToken) {
      ...
      const getCurrentParkingSessions = axios.get('/api/GarageStaff/CurrentParkingSessions', { headers });
      const getAllReservations = axios.get('/api/GarageStaff/AllReservation', { headers });
  ...
  const handleConfirmButtonClick = (plateLetters, plateNumbers) => {
    ...
    const transaction = inGarageVehicles.find((vehicle) => 
      vehicle.plateLetters === plateLetters && vehicle.plateNumbers === plateNumbers
    );
  ...
  const handleCloseConfirmPopup = async () => {
    try {
      if (!selectedPlateLetters || !selectedPlateNumbers || !selectedTransaction) {
        console.error('Error ending parking session: Selected plate number or transaction is null');
        return;
      }
      ...
      const response = await axios.delete('/api/GarageStaff/EndParkingSession', {
        data: requestData,
        headers,
      });
    ...
    <div className={`${styles.container} ${darkMode ? styles['dark-mode'] : ''}`}>
      <div className={styles['search-container']}>
        <div className={styles['search-bar']}>
          <img src={searchIconSrc} alt="Search Icon" className={styles['search-icon']} />
          <input
            type="text"
  ...
  return (
    <div key={index} className={styles['C-card']}>
      ...
      <p>Duration: {vehicle.curSessionDuration_Hours ? vehicle.curSessionDuration_Hours.toFixed(2) : 'N/A'}</p>
      <div className={styles['C-card-bottom']}>
        <h3>Total cost: {vehicle.currentBill ? vehicle.currentBill.toFixed(2) : 'N/A'} LE</h3>
        <button
          className={styles['C-confirm-button']}
          onClick={() => handleConfirmButtonClick(vehicle.plateLetters, vehicle.plateNumbers)}
      ...
      {showConfirmPopup && selectedTransaction && (
        <div className={styles['C-popup']}>
          <div className={styles['C-popup-content']}>
            <p>Plate Number: {`${selectedPlateLetters}${selectedPlateNumbers}`}</p>
            <p>Entry Time: {new Date(selectedTransaction.startDate).toLocaleString()}</p>
            ...
            <div className={styles['C-payment-options']}>
              <label htmlFor="paymentAmount" className={styles['payment-label']}>Payment Amount:</label>
              <input
                type="number"
                id="paymentAmount"
                value={paymentAmount}
